  ...
  routing {
    ...
    get("/hello") {
      println("###JKB### get /hello ....")
        ...
        if (userSession != null) {
          val rawResponse: String = httpClient.get("$keycloakAddr/auth/realms/ktor/protocol/openid-connect/userinfo") {
            headers {
              append(HttpHeaders.Authorization, "Bearer ${userSession.token}")
            }
          }.body()
          println("###JKB### response $rawResponse")

            val userInfo: UserInfo = httpClient.get("$keycloakAddr/auth/realms/ktor/protocol/openid-connect/userinfo") {
              headers {
                append(HttpHeaders.Authorization, "Bearer ${userSession.token}")
              }
            }.body()

          println("###JKB### userInfo $userInfo")
